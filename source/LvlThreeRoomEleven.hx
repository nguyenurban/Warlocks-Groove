...
class LvlThreeRoomEleven extends LevelState
{
	override public function create()
	{
		super.create();
		room_no = RoomNo.L3R11;
		FlxG.fixedTimestep = false;

		end_of_level = false;
		bgColor = 0xffcccccc;
		createLevel();
		nextLevel = LvlThreeRoomTwelve;
		currLevel = LvlThreeRoomEleven;
	...
	{
		map = new FlxOgmo3Loader(AssetPaths.map1__ogmo, AssetPaths.lvl3room11__json);
		if (LevelStats.hard_mode)
		{
			map = new FlxOgmo3Loader(AssetPaths.map1__ogmo, AssetPaths.lvl3room11hard__json);
		}
