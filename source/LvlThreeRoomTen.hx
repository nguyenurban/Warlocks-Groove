...
class LvlThreeRoomTen extends LevelState
{
	override public function create()
	{
		super.create();
		room_no = RoomNo.L3R10;
		FlxG.fixedTimestep = false;

		end_of_level = false;
		bgColor = 0xffcccccc;
		createLevel();
		nextLevel = LvlThreeRoomEleven;
		currLevel = LvlThreeRoomTen;
	...
	{
		map = new FlxOgmo3Loader(AssetPaths.map1__ogmo, AssetPaths.lvl3room10__json);
		if (LevelStats.hard_mode)
		{
			map = new FlxOgmo3Loader(AssetPaths.map1__ogmo, AssetPaths.lvl3room10hard__json);
		}
