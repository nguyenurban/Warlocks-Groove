...
class LevelState extends FlxState
{
	...
	override function update(elapsed:Float)
	{
		...
		{
			if (shortest_notes_elpsd % snpq == 0)
			{
				beat_sound.play();
			}
			if (shortest_notes_elpsd % tick_format.length == 0)
			{
				_player.barRegen();
			}
		}
		...
		prev_sne = shortest_notes_elpsd; // this must always be last thing in update()
	}
	...
	private function shoot()
	{
		...
		if (_player.alive && (FlxG.mouse.justPressed || FlxG.mouse.justPressedRight || FlxG.keys.justPressed.SPACE))
		{
			...
			{
				var timing;
				var diff = Math.abs(closest_tick.getTick() * shortest_note_len - timer) - DELAY;
				trace(diff);
	...
	private function get_closest_tick():Tick
	{
		...
		while (shortest_notes_elpsd - i >= 0 && i * shortest_note_len <= OK_WINDOW)
		{
			var curr = _ticks[(shortest_notes_elpsd - i) % _ticks.length];
		...
		i = 1;
		var later_beat = null;
		while (i * shortest_note_len <= OK_WINDOW)
		{
			var curr = _ticks[(shortest_notes_elpsd + i) % _ticks.length];
			if (curr.getJudge() == NONE)
			{
		...
		return timer - earlier_beat.getTick() * shortest_note_len <= later_beat.getTick() * shortest_note_len - timer ? earlier_beat : later_beat;
	}
	...
	private function createTexts()
	{
		...
		timer_text.text = "Time elapsed: " + timer;
		...
		timer_beats_text.text = "Beats elapsed: " + timer;
	...
	{
		timer_text.text = "Time elapsed: " + timer;
		timer_beats_text.text = "Beats elapsed: " + timer / qtr_note;
