...
class LvlThreeRoomThirteen extends LevelState
{
	override public function create()
	{
		super.create();
		room_no = RoomNo.L3R13;
		FlxG.fixedTimestep = false;

		end_of_level = false;
		bgColor = 0xffcccccc;
		createLevel();
		nextLevel = LvlThreeRoomFourteen;
		currLevel = LvlThreeRoomThirteen;
	...
	{
		map = new FlxOgmo3Loader(AssetPaths.map1__ogmo, AssetPaths.lvl3room13__json);
		if (LevelStats.hard_mode)
		{
			map = new FlxOgmo3Loader(AssetPaths.map1__ogmo, AssetPaths.lvl3room13hard__json);
		}
