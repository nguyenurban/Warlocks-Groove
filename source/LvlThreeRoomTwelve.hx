...
class LvlThreeRoomTwelve extends LevelState
{
	override public function create()
	{
		super.create();
		room_no = RoomNo.L3R12;
		FlxG.fixedTimestep = false;

		end_of_level = false;
		bgColor = 0xffcccccc;
		createLevel();
		nextLevel = LvlThreeRoomThirteen;
		currLevel = LvlThreeRoomTwelve;
	...
	{
		map = new FlxOgmo3Loader(AssetPaths.map1__ogmo, AssetPaths.lvl3room12__json);
		if (LevelStats.hard_mode)
		{
			map = new FlxOgmo3Loader(AssetPaths.map1__ogmo, AssetPaths.lvl3room12hard__json);
		}
